...
export const createProduct = async (req, res) => {
  ...
  if (req.files && req.files.length) {
    ...
    req.body.images = uploads.map((u, i) => ({
      url: u.secure_url,
...
export const updateProduct = async (req, res) => {
  ...
  if (removePublicIds && Array.isArray(removePublicIds)) {
    for (const pid of removePublicIds) {
      ...
      if (idx !== -1) {
        if (product.images[idx].public_id)
          await deleteImage(product.images[idx].public_id);
  ...
  product.narration = req.body.narration ?? product.narration;
  product.netWeight = req.body.netWeight ?? product.netWeight;
  product.grossWeight = req.body.grossWeight ?? product.grossWeight;
